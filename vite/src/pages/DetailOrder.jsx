...
export default function DetailOrder() {
  ...
  function fetchOrder() {
    const orderPromise = axios.get(apiBaseUrl(`/orders/${orderId}`), {
      headers: {
  ...
  function fetchPhotos() {
    axios
      .get(apiBaseUrl(`/orders/photos/${orderId}`), {
        headers: {
          Authorization: new Cookies().get("token"),
        },
      })
  ...
  async function handleWhatsappChat(status) {
    ...
    order.sub_orders.forEach((subOrder, index) => {
      textRequestForPickup += `\n• *Layanan* : ${subOrder?.type}\n• *Jumlah* : ${
        ...
        parseInt(subOrder.is_price_per_unit) === 1
          ? " / Unit"
          : subOrder?.price_per_multiplied_kg
          ? ` / ${subOrder?.price_per_multiplied_kg} KG`
          : " / KG"
      } \n• *Sub Total* : ${formatRupiah(subOrder?.total, "Rp. ")}\n=====`;
      textOrderDone += `\n• *Layanan* : ${subOrder?.type}\n• *Jumlah* : ${subOrder.amount}\n• *Harga* : ${formatRupiah(
        subOrder.price_per_kg,
        "Rp. "
      )} ${
        parseInt(subOrder.is_price_per_unit) === 1
          ? " / Unit"
    ...
    switch (status) {
      case "Sedang dikerjakan":
        ...
        }&text=${encodeURIComponent(textWithOrderLink)}`;
        break;
      case "Menunggu diambil":
        window.location.href = `https://api.whatsapp.com/send/?phone=${
          order.customer.phone_number
        }&text=${encodeURIComponent(textRequestForPickup)}`;
        break;
      case "Sudah diantar":
        window.location.href = `https://api.whatsapp.com/send/?phone=${
          order.customer.phone_number
        }&text=${encodeURIComponent(textDelivered)}`;
        break;
      case "Selesai":
        window.location.href = `https://api.whatsapp.com/send/?phone=${
          order.customer.phone_number
        }&text=${encodeURIComponent(textOrderDone)}`;
        break;
      default:
        break;
    }
    ...
    <MasterLayout>
      <div className="order-page">
            ...
            <>
              <div /*data-aos="flip-right"*/ className="card text-center mb-3">
                <div className="card-body text-start m-0 p-0">
                  <h5 className="card-header fw-bold text-center rounded-top p-2 border-2 border-black border-bottom bg-secondary text-white">
                    {order?.customer.name}
                  </h5>
                  <div className="p-3">
                ...
                <div className="card-footer text-muted">
                  <div>
                    Tanggal Masuk : {order && format(new Date(order?.created_at), "dd MMMM yyyy", { locale: id })}
                  </div>
                  <div>
                    Tanggal Selesai :{" "}
                    {!order.done_at ? "-" : format(new Date(order?.done_at), "dd MMMM yyyy", { locale: id })}
                  </div>
                </div>
              </div>
                  ...
                  <tbody className="table-light">
                    ...
                    <tr className="fw-semibold">
                      ...
                      <td>
                        ...
                        <div>
                          <img
                            className="img-fluid border border-secondary border-4 rounded mb-2"
                            style={{ height: "200px", objectFit: "contain" }}
                            src="/assets/pembayaran/qris-barcode-crop.jpg"
                            alt="QRIS Cinta Laundry"
                          />
                        </div>
                ...
                <div>
                  <div className="d-flex gap-2 mb-3 fw-bold">
                    <select
                      ...
                      className="form-select btn button-accent-purple"
                  ...
                  <div className="d-flex flex-column gap-3">
                    <button
                      className="btn btn button-accent-purple rounded-pill w-100"
                      data-bs-toggle="modal"
                      data-bs-target="#change-notes-modal">
                    ...
                    <button
                      className="btn btn button-accent-purple rounded-pill w-100"
                      data-bs-toggle="modal"
                      data-bs-target="#change-date-modal">
                      <i className="bi bi-check-circle"></i> Ubah Tanggal
                    </button>

                    <button
                      onClick={() => handleWhatsappChat(order?.status)}
                    ...
                    <a
                      ...
                      href={`intent:#Intent;scheme=startci://open?url_param=${
                        order?.id
                      };package=com.dantsu.thermalprinter;S.browser_fallback_url=${encodeURIComponent(
                        "https://cintalaundry.atras.my.id/#/not_found"
                      )};end`}>
        ...
        <div
          ...
          id="change-notes-modal"
          tabIndex="-1"
          aria-labelledby="change-notes-modal"
          aria-hidden="true">
          <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div className="modal-content">
              ...
              <div className="modal-footer">
                <button
                  ref={closeModalChangeNotesButton}
                  type="button"
            ...
            <div className="modal-content h-100">
              ...
              <div className="modal-body">
                <select
                  ...
                  onChange={() => setChangeDate(inputChangeDate.current.value)}>
              ...
              <div className="modal-footer">
                <button
                  id="change-date-modal-back-button"
                  type="button"
                  className="btn button-accent-purple w-100"
                  data-bs-dismiss="modal">
                  Kembali
                </button>
              </div>
            ...
            <div className="modal-content">
              ...
              <div className="modal-footer">
                <button
                  type="button"
                  className="btn btn-secondary"
                  id="button-close-upload-photo-modal"
                  data-bs-dismiss="modal">
                  Kembali
                </button>

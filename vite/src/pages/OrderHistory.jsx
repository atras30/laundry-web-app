...
export default function OrderHistory() {
  ...
  const handleInputCustomerChange = () => {
    setOrders(
      initOrders.filter((order) =>
        order.customer.name.toLowerCase().includes(inputCustomerName.current.value.toLowerCase())
      )
    );
  };
    ...
    <MasterLayout>
      <div data-aos="fade-down" className="container orders pb-2">
        ...
        <div className="d-flex gap-2 mb-3 fw-bold">
          <select
            ref={filterOrderStatus}
            onChange={handleFilterChange}
            className="form-select btn btn-secondary rounded-pill"
        ...
        {new Cookies().get("token") && (
          <button
            className="btn button-accent-purple rounded-pill w-100 mb-3 fw-bold"
            style={{ background: "#287eff" }}
